...
describe('connection node half', () => {
  ...
  it('allows privileged methods from a declared trusted authority', async () => {
    const { routes, dispose } = await mounted({ trustedHosts: ['harness.example'] })
    // Privileged methods are no longer pinned to loopback; a declared trusted
    // authority reaches them through the bridge (carrier-level 404 from the
    // empty proxy proves the fence passed).
    for (const method of [
...
describe('connection node half over a real HTTP server', () => {
  ...
  it('answers a declared LAN authority with 200 on all methods, over real HTTP', async () => {
    // Privileged methods are no longer pinned to loopback; a declared trusted
    // authority reaches everything through the bridge.
    const { routes, dispose } = await mounted({ trustedHosts: ['harness.example'] })
    ...
    try {
      ...
      // The model catalog stays reachable for the same authority.
      for (const method of ['llm.providers', 'llm.models', 'agentPreset.list', 'agentPreset.select']) {
        expect([method, await call(port, method, 'harness.example')]).toEqual([method, 404])
      }
      // Loopback reaches everything too.
